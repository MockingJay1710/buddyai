                ...
                if hasattr(module, "COMMANDS") and isinstance(module.COMMANDS, dict):
                    for command_name, command_function in module.COMMANDS.items():
                        if command_name in COMMAND_REGISTRY:
                            print(
                                f"    Warning: Command '{command_name}' from module '{module_import_name}' is overwriting an existing command."
                            )
                        COMMAND_REGISTRY[command_name] = command_function
                        print(
                            f"    Registered command: '{command_name}' -> {command_function.__name__}"
                        )
                else:
                    print(
                        f"    Module '{module_import_name}' does not have a 'COMMANDS' dictionary or it's not a dict."
                    )
            except ImportError as e:
                print(f"    Failed to import module '{module_import_name}'. Error: {e}")
                print(
                    f"    Check if '{modules_package_name}' is a proper package and accessible in sys.path."
                )
            except Exception as e:
                print(
                    f"    An unexpected error occurred while processing module '{module_import_name}'. Error: {e}"
                )

    print(
        f"Finished loading modules. Available commands: {list(COMMAND_REGISTRY.keys())}"
    )
...
def get_commands_schema():
    ...
    for command_name, command_function in COMMAND_REGISTRY.items():
        ...
        description = (
            func_doc.split("\n\n")[0] if func_doc else "No description available."
        ...
        try:
            ...
            if command_name in COMMAND_REGISTRY:
                try:
                    ...
                    TypeError
                ) as e:  # Catch errors if params don't match function signature
                    return jsonify(
                        {
                            "status": "error",
                            "message": f"Parameter mismatch for command '{command_name}': {str(e)}",
                        }
                    ), 400
                except (
                    Exception
                ) as e:  # Catch any other errors during command execution
                    return jsonify(
                        {
                            "status": "error",
